...
func (c call) Eval(env Env) float64 {
	...
	case "min":
		return math.Min(c.args[0].Eval(env), c.args[1].Eval(env)) // Answer to Q7.14
	}
...
// Answer to Q7.14 (Add post Unary factorial operator that satisfies Expr interface)
type postUnary struct {
	op rune // one of '+', '-'
	x Expr
}

func (u postUnary) Eval(env Env) float64 {
	switch u.op {
	case '!':
		// gamma[n+1] = n!
		return math.Gamma(u.x.Eval(env) + 1)
	}
	panic(fmt.Sprintf("unsupported post unary operator: %q", u.op))
}

func (u postUnary) String() string {
	switch u.op {
	case '!':
		return fmt.Sprintf("%s%s", u.x, string(u.op))
	}
	panic(fmt.Sprintf("unsupported post unary operator: %q", u.op))
}